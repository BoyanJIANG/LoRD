...
class MeshEvaluator(object):
    ...
    def eval_mesh(self, mesh_pred, mesh_gt):
        ''' Evaluates a mesh.
        Args:
        ...
        '''
        pointcloud, idx = mesh_pred.sample(self.n_points, return_index=True)
        pointcloud = pointcloud.astype(np.float32)
        normals = mesh_pred.face_normals[idx]

        pointcloud_tgt, idx_tgt = mesh_gt.sample(self.n_points, return_index=True)
        pointcloud_tgt = pointcloud_tgt.astype(np.float32)
        normals_tgt = mesh_gt.face_normals[idx_tgt]
    ...
    def eval_pointcloud(self, pointcloud, pointcloud_tgt,
                        normals=None, normals_tgt=None, threshold=0.005):
        ...
        precision_1 = np.mean((completeness < threshold).astype(np.float32))
        ...
        precision_2 = np.mean((accuracy < threshold).astype(np.float32))
...
def distance_p2p(points_src, normals_src, points_tgt, normals_tgt):
    ...
    if normals_src is not None and normals_tgt is not None:
        ...
        normal_cons = np.linalg.norm(normals_tgt[idx]-normals_src, axis=-1)
